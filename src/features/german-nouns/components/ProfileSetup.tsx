...
export function ProfileSetup({ onProfileCreated }: ProfileSetupProps) {
    ...
    async function loadLanguages() {
      try {
        const res = await fetchWithAuth(
          germanNounsUrl(germanNounsEndpoints.languages.native),
          { method: 'GET' },
          token,
        )
        if (!res.ok) {
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetchWithAuth(
        germanNounsUrl(germanNounsEndpoints.profile.create),
        {
          method: 'POST',
  ...
  return (
    <div className="rounded border border-slate-200 bg-white p-6 shadow-sm">
      <h2 className="mb-2 text-lg font-semibold text-slate-800">Welcome to German Nouns</h2>
