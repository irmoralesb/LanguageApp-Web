...
interface IdiomCompleteExerciseViewProps {
...
export function IdiomCompleteExerciseView({
  selections,
  onOpenSelector,
}: IdiomCompleteExerciseViewProps) {
  const { token } = useAuth()
  ...
  const generateExercise = useCallback(async () => {
    ...
    try {
      const res = await fetchWithAuth(
        englishUrl(expressionsEndpoints.exercises.idiomComplete.generate),
        {
          method: 'POST',
  ...
  const handleCheck = useCallback(async () => {
    ...
    try {
      const res = await fetchWithAuth(
        englishUrl(expressionsEndpoints.exercises.idiomComplete.evaluate),
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-teal-700">Idiom Complete</p>
          <h2 className="text-xl font-bold text-slate-900">Fill in the idiom</h2>
        </div>
