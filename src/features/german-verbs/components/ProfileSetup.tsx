...
export function ProfileSetup({ onProfileCreated }: ProfileSetupProps) {
    ...
    async function loadLanguages() {
      try {
        const res = await fetchWithAuth(
          germanVerbsUrl(germanVerbsEndpoints.languages.native),
          { method: 'GET' },
          token,
        )
        if (!res.ok) {
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetchWithAuth(
        germanVerbsUrl(germanVerbsEndpoints.profile.create),
        {
          method: 'POST',
      ...
      onProfileCreated(await res.json())
    } catch {
  ...
  if (loading) return <p className="text-slate-500">Loading languages...</p>

  return (
    <div className="rounded border border-slate-200 bg-white p-6 shadow-sm">
      <h2 className="mb-2 text-lg font-semibold text-slate-800">Welcome to German Verbs</h2>
      <p className="mb-6 text-slate-600">Select your native language to receive prompts in that language.</p>
      {error && (
      ...
      <select
        ...
        className="mb-6 block w-full rounded border border-slate-300 px-3 py-2"
      >
      ...
      <button
        ...
        className="rounded bg-slate-700 px-5 py-2 text-white hover:bg-slate-600 disabled:opacity-50"
      >
