...
interface PrepositionChoiceExerciseViewProps {
        ...
        <span key={`${part}-${index}`} className="inline-flex flex-wrap items-center gap-2">
          ...
          {index < parts.length - 1 ? (
            <input
              ...
              disabled={disabled}
              aria-label="Preposition answer"
              placeholder="?"
              className="w-24 rounded-lg border border-teal-300 bg-white px-2 py-1 text-center text-base font-semibold text-teal-900 focus:border-teal-500 focus:outline-none focus:ring-1 focus:ring-teal-500 disabled:bg-slate-50"
            />
          ) : null}
        </span>
      ))}
...
export function PrepositionChoiceExerciseView({
  selections,
  onOpenSelector,
}: PrepositionChoiceExerciseViewProps) {
  const { token } = useAuth()

  const [exercise, setExercise] = useState<PrepositionChoiceExercisePromptResponse | null>(null)
  const [userAnswer, setUserAnswer] = useState('')
  const [evaluation, setEvaluation] = useState<PrepositionChoiceEvaluationResponse | null>(null)
  const [pairStats, setPairStats] = useState<PrepositionChoicePairStatsResponse[]>([])
  ...
  const loadStats = useCallback(async () => {
    try {
      const res = await fetchWithAuth(
        prepositionsUrl(prepEndpoints.exercises.prepositionChoice.stats),
        { method: 'GET' },
        token,
      )
      if (res.ok) {
      ...
      /* stats are optional for the exercise flow */
    }
  }, [token])
  ...
  const generateExercise = useCallback(async () => {
    ...
    try {
      const res = await fetchWithAuth(
        prepositionsUrl(prepEndpoints.exercises.prepositionChoice.generate),
        {
          method: 'POST',
      ...
      const data: PrepositionChoiceExercisePromptResponse = await res.json()
      setExercise(data)
      await loadStats()
    } catch {
  ...
  const handleCheck = useCallback(async () => {
    ...
    try {
      const res = await fetchWithAuth(
        prepositionsUrl(prepEndpoints.exercises.prepositionChoice.evaluate),
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      const data: PrepositionChoiceEvaluationResponse = await res.json()
      setEvaluation(data)
      await loadStats()
    } catch {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-teal-700">Similar Prepositions</p>
          <h2 className="text-xl font-bold text-slate-900">Choose the correct word</h2>
        </div>
        <div className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">
          {selections.length} saved term{selections.length !== 1 ? 's' : ''}
        </div>
      </div>
