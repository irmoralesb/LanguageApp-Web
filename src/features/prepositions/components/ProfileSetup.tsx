...
export function ProfileSetup({ onProfileCreated }: ProfileSetupProps) {
    ...
    async function loadLanguages() {
      try {
        const res = await fetchWithAuth(
          prepositionsUrl(prepEndpoints.languages.native),
          { method: 'GET' },
          token,
        )
        if (!res.ok) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const res = await fetchWithAuth(
        prepositionsUrl(prepEndpoints.profile.create),
        {
          method: 'POST',
  ...
  return (
    <div className="rounded border border-slate-200 bg-white p-6 shadow-sm">
      <h2 className="mb-2 text-lg font-semibold text-slate-800">Welcome to Prepositions!</h2>
      ...
      <label htmlFor="prep-native-language" className="mb-1 block text-sm font-medium text-slate-700">
        Native Language
      </label>
      <select
        id="prep-native-language"
